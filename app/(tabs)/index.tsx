...
import { addComment, checkIfUserLiked, createPost, deletePost, getCurrentUserProfile, getPostCommentCount, getPostComments, getPostLikes, getPosts, getUsersWhoLiked, likePost, searchUsers, unlikePost } from '../../lib/supabase';
...
export default function FeedScreen() {
      ...
      if (error) {
        ...
        const postsWithData = await Promise.all(
          limitedPosts.map(async (post) => {
            const [likesData, userLikedData, commentCountData] = await Promise.all([
  ...
  const renderPost = ({ item }: { item: Post }) => (
    <View style={{ backgroundColor: colors.surface }} className="mb-6 mx-4 rounded-2xl shadow-sm overflow-hidden">
      {/* Post Header */}
      ...
      <View className="flex-row items-center justify-between px-4 pb-4">
        <TouchableOpacity 
          ...
          style={{ backgroundColor: item.is_liked ? 'rgba(239, 68, 68, 0.1)' : 'transparent' }}
        >
          <Ionicons 
          ...
          <TouchableOpacity 
            ...
            className="ml-1"
          >
            <Text style={{ color: item.is_liked ? colors.error : colors.textSecondary }} className="font-medium">
              {item.likes_count || 0}
            </Text>
          </TouchableOpacity>
        </TouchableOpacity>
        ...
        >
          ...
          <Text style={{ color: colors.textSecondary }} className="ml-1 font-medium">{item.comments_count || 0}</Text>
        </TouchableOpacity>
        ...
        >
          ...
          <Text style={{ color: colors.textSecondary }} className="ml-1 font-medium">Share</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={{ backgroundColor: colors.background }} className="flex-1">
      {/* Header with Search */}
      <View style={{ backgroundColor: colors.surface }} className="px-4 py-4 mb-3">
        <View className='flex flex-row justify-between items-center gap-2'>
        <Text style={{ color: colors.text }} className="text-2xl font-bold mb-2">Feed</Text>
        
        {/* Search Bar */}
        <View style={{ backgroundColor: colors.background }} className="flex-row w-4/5 items-center rounded-2xl px-4 py-1 shadow-sm">
        ...
        {showSearchResults && searchResults.length > 0 && (
          <View style={{ backgroundColor: colors.surface }} className="mt-2 rounded-2xl shadow-sm overflow-hidden">
            <FlatList
              data={searchResults}
      ...
      {/* Create Post Section */}
      <View style={{ backgroundColor: colors.surface }} className="mx-4 mb-3 p-4 rounded-2xl shadow-sm">
            ...
            <View className="flex-row justify-between items-center mt-3">
              <View className="flex-row space-x-4">
                <TouchableOpacity className="flex-row items-center">
                  ...
                  <Text style={{ color: colors.primary }} className="ml-1 font-medium">Photo</Text>
                </TouchableOpacity>
                <TouchableOpacity className="flex-row items-center">
                  <Ionicons name="camera" size={20} color={colors.success} />
                  <Text style={{ color: colors.success }} className="ml-1 font-medium">Camera</Text>
                </TouchableOpacity>
              </View>
              <TouchableOpacity
                style={{ 
                  backgroundColor: posting || !newPostContent.trim() ? colors.textSecondary : colors.primary,
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="flex-1 justify-center items-center py-20">
            ...
            <Text style={{ color: colors.textSecondary }} className="text-lg mt-4 text-center font-medium">
              {loading ? 'Loading posts...' : 'No posts from friends yet. Add some friends to see their posts!'}
            </Text>
            {!loading && (
              <TouchableOpacity 
                style={{ backgroundColor: colors.primary }}
      ...
      >
        <SafeAreaView style={{ backgroundColor: colors.background }} className="flex-1">
          <View style={{ backgroundColor: colors.surface }} className="flex-row items-center justify-between p-4">
            ...
            <Text style={{ color: colors.text }} className="text-lg font-semibold">Comments</Text>
            <View style={{ width: 24 }} />
          </View>
              ...
              <View className="py-4">
                <View className="flex-row items-start">
                  ...
                  <View className="flex-1">
                    ...
                    <Text style={{ color: colors.text }} className="mt-1">{item.content}</Text>
          ...
          <View style={{ backgroundColor: colors.surface }} className="p-4">
            <View className="flex-row items-center">
              ...
              <TouchableOpacity
                ...
                style={{ 
                  backgroundColor: commenting || !commentText.trim() ? colors.textSecondary : colors.primary,
      ...
      >
        <SafeAreaView style={{ backgroundColor: colors.background }} className="flex-1">
          <View style={{ backgroundColor: colors.surface }} className="flex-row items-center justify-between p-4">
            ...
            <Text style={{ color: colors.text }} className="text-lg font-semibold">Likes ({usersWhoLiked.length})</Text>
            <View style={{ width: 24 }} />
          </View>

...
const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImV3c29qYWlwZmt1ZWt5YndqbW9lIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA0Nzc2NTUsImV4cCI6MjA2NjA1MzY1NX0.3fKMgkaJgWvA-deSiTV9VKubBus8-yBrwrAun-ud52Q';
  ...
  try {
    ...
    try {
      ...
      if (profileError) {
        ...
        return { data: { user: authData.user, profile: manualProfile }, error: null };
      }
...
export const rejectFriendRequest = async (senderId: string) => {
  try {
    ...
    return { data: null, error: { message: 'Failed to reject friend request' } };
  }
};
...
export const unfriendUser = async (friendId: string) => {
  try {
    ...
    if (checkError) {
      ...
      console.log('Existing relationship before unfriend:', existingRelationship);
    }
...
export const getFriendRequestStatus = async (targetUserId: string) => {
  try {
    ...
    console.log('Checking friend status for user:', targetUserId, 'from user:', user.id);
...
export const getPendingFriendRequests = async () => {
  try {
    ...
    return { data: null, error: { message: 'Failed to get pending friend requests' } };
  }
};
...
export const deleteUser = async (userId: string) => {
  try {
    ...
    console.log('User data deleted. Auth account deletion requires admin privileges.');
...
export const getUsersWhoLiked = async (postId: string) => {
  try {
    ...
    return { data: null, error: { message: 'Failed to fetch users who liked' } };
  }
};
...
export const deletePost = async (postId: string) => {
  try {
    ...
    if (!existingPost) {
      ...
      return { error: { message: 'Post not found or you do not have permission to delete it' } };
    }
    ...
    const { data: functionResult, error: functionError } = await supabase
      .rpc('delete_post_simple', { post_id: postId });
    ...
    if (functionResult && functionResult.includes('Post deleted successfully')) {
      // Verify the post is actually deleted
      const { data: verifyResult, error: verifyError } = await supabase
        .rpc('check_post_exists', { post_id: postId });
...
export const checkPostExists = async (postId: string) => {
  try {
    ...
    return { exists: false, data: null, error: { message: 'Failed to check post existence' } };
  }
};
...
export const getAllPostsDebug = async () => {
  try {
    ...
    data?.forEach((post: any, index: number) => {
      console.log(`${index + 1}. ID: ${post.id}, Content: ${post.content.substring(0, 50)}..., User: ${post.user_id}, Created: ${post.created_at}`);
    });
